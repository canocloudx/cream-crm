...
app.delete('/api/members/:id', async (req, res) => {
    try {
        ...
        // First delete any reward history for this member
        await pool.query('DELETE FROM reward_history WHERE member_id = $1', [id]);
...
app.put('/api/members/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, email, stamps, availableRewards } = req.body;
        
        const result = await pool.query(
            `UPDATE members 
             SET name = $1, email = $2, stamps = $3, available_rewards = $4, updated_at = NOW()
             WHERE id = $5 RETURNING *`,
            [name, email, stamps, availableRewards, id]
        );
...
console.log('✏️ Edit/Delete member endpoints loaded');
